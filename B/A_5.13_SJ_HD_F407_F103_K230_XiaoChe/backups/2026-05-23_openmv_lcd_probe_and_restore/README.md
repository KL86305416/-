...
4. If the LCD shows abnormal colors/orientation, run `lcd_probe.py` and note which mode looks correct.
5. Tune these LCD parameters in `main.py` if needed:
   - `LCD_BGR`
   - `LCD_BYTE_SWAP`
   - `LCD_HMIRROR`
   - `LCD_VFLIP`
6. Wire `UART7` to the F407 board:
   - `OpenMV P14 (UART7 TX)` -> `F407 OpenMV-TX / UART4_RX`
   - `OpenMV P13 (UART7 RX)` -> `F407 OpenMV-RX / UART4_TX`
   - `GND` -> `GND`
7. Upload `main.py` to the OpenMV N6 board.
