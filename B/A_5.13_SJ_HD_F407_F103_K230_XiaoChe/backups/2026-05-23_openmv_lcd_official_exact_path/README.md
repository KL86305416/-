...
4. If the LCD is heavily garbled, run `lcd_min_test.py` first.
5. If the LCD shows abnormal colors/orientation but is still basically readable, run `lcd_probe.py` and note which mode looks correct.
6. Tune these LCD parameters in `main.py` if needed:
   - `LCD_BGR`
   - `LCD_BYTE_SWAP`
   - `LCD_HMIRROR`
   - `LCD_VFLIP`
   - `LCD_REFRESH`
7. Wire `UART7` to the F407 board:
   - `OpenMV P14 (UART7 TX)` -> `F407 OpenMV-TX / UART4_RX`
   - `OpenMV P13 (UART7 RX)` -> `F407 OpenMV-RX / UART4_TX`
   - `GND` -> `GND`
8. Upload `main.py` to the OpenMV N6 board.
...
- `lcd_min_test.py` is the first-line self-test for a heavily garbled official screen.
- `lcd_probe.py` is a dedicated LCD parameter probe for abnormal official-screen output.
