...
def classify_red_circle(frame):
    ...
    radius = int((blob_w + blob_h) / 4)
    x_offset = int(blob_cx - (frame.width() / 2))
    flags = VISION_FLAG_VALID | VISION_FLAG_RED_FOUND

    if (roundness >= 0.55) and (density >= 0.55):
        flags |= VISION_FLAG_CIRCLE_LIKE

    return {
        "blob": best_blob,
        "cx": blob_cx,
        "cy": get_member_value(best_blob, "cy"),
        "rect": get_member_value(best_blob, "rect"),
        "x_offset": x_offset,
        "radius": radius,
        "flags": flags,
        "score": best_score,
...
def main():
    ...
    while True:
        ...
        if result is not None:
            ...
            frame.draw_circle((blob_cx, blob_cy, radius), color=(0, 255, 0), thickness=2)
